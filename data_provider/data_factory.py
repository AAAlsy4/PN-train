...
class Dataset:
    ...
    def _get_datasets_ho_ge(self, start, end, type):
        ...
        for curr in range(start, end):
            ...
            # 计算节假日时间步数量
            holiday_count_x = np.sum(s_x_mark[..., self.ho_index])
            holiday_count_y = np.sum(s_y_mark[..., self.ho_index])
            total_holiday_count = holiday_count_x + holiday_count_y

            if total_holiday_count >= 12:
                seq_x_ho.append(s_x)
                seq_y_ho.append(s_y)
