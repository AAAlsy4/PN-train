...
class Exp(Exp_Basic):
    ...
    def finetune(self, setting, logger):
        ...
        # 3. 第一阶段：对假期神经元进行微调（冻结其他神经元）
        if self.args.finetune_sample_num != 0:
            self.logger.info("第一阶段：对假期神经元进行微调（冻结其他神经元）")

            # 创建优化器，只对可训练参数进行优化
            trainable_params = []
            for name, param in holiday_model.named_parameters():
                match = re.search(r'layers_(\w)\.(\d+)', name)
                if match:
                    layer_type = match.group(1)
                    layer_num = int(match.group(2))
                    # 检查是否是假期神经元
                    if self._is_holiday_neuron(name, layer_type, layer_num):
                        trainable_params.append(param)
                        param.requires_grad = True
                    else:
                        param.requires_grad = False  # 冻结非假期神经元
                else:
                    param.requires_grad = False  # 冻结其他参数

            # 创建优化器（只对可训练参数）
            if trainable_params:
                opt = optim.AdamW(trainable_params, lr=self.args.finetune_learning_rate)
            else:
                opt = None
            ...
            if opt:
                ...
                for epoch in range(self.args.finetune_epochs):
                    total_loss = 0
                    for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(self.retrain_loader_ho):
                        opt.zero_grad()

                        pred, true = self._process_one_batch_model(holiday_model, batch_x, batch_y, batch_x_mark,
                                                                   batch_y_mark)
                    ...
                    avg_loss = total_loss / len(self.retrain_loader_ho)
                    self.logger.info(f"微调阶段 - Epoch {epoch + 1}/{self.args.finetune_epochs}, Loss: {avg_loss:.4f}")

            # 测试微调后的模型在假期数据上的表现
            self.test_model(holiday_model, self.debug_loader_ho,
                            log_info='假期数据微调后',
                            log_name='holiday_after_finetune')

        # 4. 第二阶段：解冻所有神经元，使用全类型样本重新训练
        self.logger.info("第二阶段：解冻所有神经元，使用全类型样本重新训练")

        # 解冻所有参数
        for param in holiday_model.parameters():
            param.requires_grad = True

        # 创建新的优化器（对所有参数）
        full_opt = optim.AdamW(holiday_model.parameters(), lr=self.args.learning_rate)

        # 定义训练轮数（可根据需要调整）
        full_train_epochs = 20  # 可根据需要调整为参数

        # 准备全类型数据加载器（合并假期和常规数据）
        # 这里我们需要创建一个合并的数据加载器
        from torch.utils.data import ConcatDataset, DataLoader

        # 获取假期和常规数据集
        holiday_data = []
        for batch in self.retrain_loader_ho:
            holiday_data.append(batch)

        general_data = []
        for batch in self.retrain_loader_ge:
            general_data.append(batch)

        # 合并数据集（这里简化处理，实际可能需要更复杂的数据合并）
        # 注意：这里假设retrain_loader_ho和retrain_loader_ge有相同结构
        ...
        for epoch in range(full_train_epochs):
            ...
            # 交替训练假期和常规数据
            for i in range(max(len(self.retrain_loader_ho), len(self.retrain_loader_ge))):
                full_opt.zero_grad()

                # 交替使用假期和常规数据
                if i % 2 == 0 and i < len(self.retrain_loader_ho):
                    # 使用假期数据
                    batch_x, batch_y, batch_x_mark, batch_y_mark = next(iter(self.retrain_loader_ho))
                elif i < len(self.retrain_loader_ge):
                    # 使用常规数据
                    batch_x, batch_y, batch_x_mark, batch_y_mark = next(iter(self.retrain_loader_ge))
                else:
                    continue

                pred, true = self._process_one_batch_model(holiday_model, batch_x, batch_y, batch_x_mark, batch_y_mark)
            ...
            if batch_count > 0:
                ...
                self.logger.info(f"全类型训练 - Epoch {epoch + 1}/{full_train_epochs}, Loss: {avg_loss:.4f}")

        # 5. 测试最终模型
        self.logger.info("最终模型测试")
        self.test_model(holiday_model, self.debug_loader_ho,
                        log_info='假期数据最终',
                        log_name='holiday_final')
        self.test_model(holiday_model, self.debug_loader_ge,
                        log_info='常规数据最终',
                        log_name='general_final')
        self.test_model(holiday_model, self.test_loader,
                        log_info='全部数据最终',
                        log_name='all_final')

        # 6. 保存模型
        model_path = os.path.join(path, 'final_model.pth')
        torch.save(holiday_model.state_dict(), model_path)
        self.logger.info(f"模型已保存到: {model_path}")

        return holiday_model

    def _is_holiday_neuron(self, param_name, layer_type, layer_num):
        """判断给定的参数是否是假期神经元"""
        # 从已检测的假期神经元索引中查找
        if layer_type == 't':
            if 'attn.FC_Q.weight' in param_name:
                return layer_num in self.q_t_index_ho_new and len(self.q_t_index_ho_new[layer_num]) > 0
            elif 'attn.FC_K.weight' in param_name:
                return layer_num in self.k_t_index_ho_new and len(self.k_t_index_ho_new[layer_num]) > 0
            elif 'attn.FC_V.weight' in param_name:
                return layer_num in self.v_t_index_ho_new and len(self.v_t_index_ho_new[layer_num]) > 0
            elif 'attn.out_proj.weight' in param_name:
                return layer_num in self.out_t_index_ho_new and len(self.out_t_index_ho_new[layer_num]) > 0
            elif 'feed_forward_up.0.weight' in param_name:
                return layer_num in self.up_t_index_ho_new and len(self.up_t_index_ho_new[layer_num]) > 0
            elif 'feed_forward_down.weight' in param_name:
                return layer_num in self.down_t_index_ho_new and len(self.down_t_index_ho_new[layer_num]) > 0
        elif layer_type == 's':
            if 'attn.FC_Q.weight' in param_name:
                return layer_num in self.q_s_index_ho_new and len(self.q_s_index_ho_new[layer_num]) > 0
            elif 'attn.FC_K.weight' in param_name:
                return layer_num in self.k_s_index_ho_new and len(self.k_s_index_ho_new[layer_num]) > 0
            elif 'attn.FC_V.weight' in param_name:
                return layer_num in self.v_s_index_ho_new and len(self.v_s_index_ho_new[layer_num]) > 0
            elif 'attn.out_proj.weight' in param_name:
                return layer_num in self.out_s_index_ho_new and len(self.out_s_index_ho_new[layer_num]) > 0
            elif 'feed_forward_up.0.weight' in param_name:
                return layer_num in self.up_s_index_ho_new and len(self.up_s_index_ho_new[layer_num]) > 0
            elif 'feed_forward_down.weight' in param_name:
                return layer_num in self.down_s_index_ho_new and len(self.down_s_index_ho_new[layer_num]) > 0
